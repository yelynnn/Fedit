      ...
      <div className="flex flex-col w-full p-4 overflow-hidden bg-fill-bg-strong rounded-xl">
        ...
        <div className="flex flex-col gap-3">
          {chartData.map((item, index) => (
            <div
              key={`${item.name}-${index}`}
              className="relative flex items-center justify-between h-12 px-3 bg-white border border-line-alt rounded-2xl overflow-hidden"
            >
              <div
                className="absolute left-0 top-0 h-full bg-brand-subtle transition-all duration-500 ease-out"
