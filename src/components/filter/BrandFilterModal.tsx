...
export default function BrandFilterModal({ isOpen, onClose, onSubmit }: Props) {
  ...
  // (interestBrandPicks) 기준으로 판단해야 한다. brandList로 판단하면
  // 그중 일부만 보려고 체크를 풀었을 때 그 브랜드가 잠겨서 다시 못 고르게 된다.
  ...
  const handleConfirmApplyBrand = async () => {
    ...
    try {
      ...
      applyToastTimerRef.current = setTimeout(
        ...
        <div className="flex items-center justify-between w-full gap-4 p-3 rounded-lg bg-fill-bg-strong">
          ...
          >
            ...
            <span className="text-xs font-semibold text-tx-neutral">
              {isApplyingBrand ? "신청 중..." : "신규 브랜드 신청하기"}
            </span>
          </button>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-[10000] flex items-center justify-center bg-black/50">
          <div className="flex w-[480px] flex-col items-center gap-5 rounded-2xl bg-white p-8 shadow-[0_8px_32px_0_rgba(0,0,0,0.16)]">
            ...
            <div className="flex w-full gap-3">
              ...
              <button
                type="button"
                onClick={handleConfirmApplyBrand}
                disabled={isApplyingBrand}
