...
function MainColorBox() {
  ...
  if (!colorData) {
    return (
      <section className="box-border px-6 py-5 w-109 h-114 rounded-xl border border-line-divider flex items-center justify-center bg-white">
  ...
  return (
    <section className="box-border px-6 py-5 w-109 h-114 rounded-xl border border-line-divider flex flex-col gap-4 bg-white">
      <div className="flex items-center gap-2 ">
        <div className="flex items-center justify-center w-8 h-8 rounded-lg bg-falling-bg">
          <Icon
      ...
      <div className="flex gap-5">
        {colors.map((item) => (
          <div key={item.color_name} className="flex items-center gap-2">
            ...
            />
            <span className="text-sm text-tx-alt">{item.color_name}</span>
          </div>
        ))}
      </div>
      ...
      <div className="border-t border-line-divider">
        <div className="flex items-center gap-1 mt-4 mb-3">
          <Icon
            ...
            className="w-4 h-4"
          />
          <p className="text-sm font-semibold text-tx-alt">
            현장에서 포착된 컬러 트렌드
          </p>
        </div>
